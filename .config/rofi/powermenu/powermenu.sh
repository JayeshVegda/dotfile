...
case ${chosen} in
    ...
    $lock)
		if [[ -x '/usr/bin/betterlockscreen' ]]; then
			...
			i3lock
		elif [[ -x '/usr/bin/light-locker-command' ]]; then
			...
			notify-send "Power Menu" "No lock screen found. Install: sudo pacman -S swaylock" 2>/dev/null || echo "No lock screen found. Install: sudo pacman -S swaylock"
		fi
        ;;
    $suspend)
		run_cmd --suspend
